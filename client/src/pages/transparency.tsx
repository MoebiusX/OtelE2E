...
const SERVICE_DISPLAY_NAMES: Record<
  string,
...
export default function TransparencyPage() {
  ...
  } = useQuery<{ services: ServiceHealth[] }>({
  ...
  const calculateSystemStatus = () => {
    ...
    if (criticalCount > 0)
      return { status: 'Issues Detected', color: 'bg-red-500', text: 'text-red-400' };
    if (warningCount > 0)
      return { status: 'Minor Issues', color: 'bg-yellow-500', text: 'text-yellow-400' };
    if (healthyCount === healthData.services.length)
      return {
  ...
  const avgResponseTime = healthData?.services?.reduce((sum, s) => sum + s.avgDuration, 0)
    ? Math.round(
        healthData.services.reduce((sum, s) => sum + s.avgDuration, 0) / healthData.services.length,
      )
    : 0;
  ...
  return (
    <Layout>
      <div className="container mx-auto px-4 py-8">
        ...
        <Card
          className={`mb-8 border-2 ${systemStatus.color}/30 bg-gradient-to-r from-slate-900 to-slate-800`}
        >
              ...
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <p className="text-cyan-100/60 text-sm">
                    {healthData?.services?.length || 0} services monitored • Avg response:{' '}
                  ...
                  <CardContent className="p-5">
                    <div className="flex items-start justify-between mb-3">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <h3 className="font-semibold text-cyan-100">{displayInfo.name}</h3>
                          <p className="text-xs text-cyan-100/50">{displayInfo.description}</p>
                        </div>
                      </div>
          ...
          <CardContent>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              <div>
                ...
                <p className="text-cyan-100/60 text-sm">
                  Each order generates a unique trace ID that tracks its journey from your click to
                  execution. View this in your Activity page.
                </p>
              </div>
              <div>
                <h4 className="font-semibold text-cyan-100 mb-2">System Performance</h4>
                <p className="text-cyan-100/60 text-sm">
                  We continuously measure response times and detect anomalies. If something slows
                  down, we know immediately.
                </p>
              </div>
              <div>
                <h4 className="font-semibold text-cyan-100 mb-2">Service Health</h4>
                <p className="text-cyan-100/60 text-sm">
                  All microservices are monitored 24/7. Issues are detected and addressed before
            ...
            <div className="mt-6 pt-6 border-t border-cyan-500/20 text-center">
              <p className="text-cyan-100/70 text-sm">
                This is <strong className="text-cyan-400">Proof of Observability™</strong> - not
                just promises, but real-time visibility into how we handle your assets.
              </p>
            </div>
          </CardContent>
        </Card>
