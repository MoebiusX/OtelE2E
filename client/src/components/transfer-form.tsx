...
export function TransferForm() {
      ...
      toast({
        ...
        description: (
          <div className="space-y-1 text-sm">
            ...
            <p className="text-xs text-slate-400">Transfer ID: {data.transferId}</p>
          </div>
        ),
      });
  ...
  const selectedRecipient = availableRecipients.find(
    (u) => u.walletAddress === form.watch('toAddress') || u.id === form.watch('toUserId'),
  );
          ...
          <div className="text-center">
            ...
            <div className="text-xs text-slate-400 font-mono">
              {selectedRecipient?.walletAddress
                ? formatAddress(selectedRecipient.walletAddress)
                : 'Recipient'}
            </div>
          </div>
        </div>
        ...
        {!usersLoading && availableRecipients.length === 0 && (
          <div className="p-6 bg-gradient-to-br from-amber-900/20 to-orange-900/20 border border-amber-500/30 rounded-xl text-center">
            ...
            <p className="text-amber-200/60 text-sm max-w-xs mx-auto">
              P2P transfers require other registered users. Create another account in a different
              browser to test this feature.
            </p>
          </div>
        )}
