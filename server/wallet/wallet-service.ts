...
export const walletService = {
  ...
  async getWallets(userId: string): Promise<Wallet[]> {
    const result = await db.query(`SELECT * FROM wallets WHERE user_id = $1 ORDER BY asset`, [
      userId,
    ]);
  ...
  async getWallet(userId: string, asset: string): Promise<Wallet | null> {
    const result = await db.query(`SELECT * FROM wallets WHERE user_id = $1 AND asset = $2`, [
      userId,
      asset.toUpperCase(),
    ]);
  ...
  async getBalanceSummary(
    userId: string,
  ): Promise<Record<string, { balance: string; available: string; locked: string }>> {
  ...
  async transfer(
    ...
    return db.transaction(async (client) => {
      ...
      const updatedSender = await client.query(`SELECT available FROM wallets WHERE id = $1`, [
        senderWallet.id,
      ]);
      const updatedReceiver = await client.query(`SELECT available FROM wallets WHERE id = $1`, [
        receiverWallet.id,
      ]);
