...
import type { TimeBaseline, AdaptiveThresholds, JaegerTrace, JaegerSpan } from './types';
...
export class BaselineCalculator {
  ...
  async recalculate(): Promise<{
    ...
    try {
      ...
      for (const service of MONITORED_SERVICES) {
        ...
        allSpans.push(...spans);
        logger.info(
          { service, tracesCount: traces.length, spansCount: spans.length },
          'Collected traces for service',
        );
      }
      ...
      logger.info(
        { baselinesCount: newBaselines.size, durationMs: duration },
        'Calculated time-aware baselines',
      );
  ...
  loadFromStorage(): void {
    ...
    if (baselines && baselines.length > 0) {
      ...
      logger.info(
        { baselinesCount: this.timeBaselines.size },
        'Loaded time baselines from storage',
      );
    }
  }
}
