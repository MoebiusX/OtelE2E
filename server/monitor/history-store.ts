  ...
  private load(): void {
    try {
      if (fs.existsSync(HISTORY_FILE)) {
        ...
        logger.info(
          {
            anomaliesCount: this.history.anomalies.length,
            baselinesCount: this.history.baselines.length,
          },
      ...
      if (fs.existsSync(TIME_BASELINES_FILE)) {
        ...
        logger.info(
          { timeBaselinesCount: this.history.timeBaselines?.length || 0 },
          'Loaded time baselines from file',
        );
      }
    } catch (error: unknown) {
  ...
  private saveTimeBaselines(): void {
    try {
      if (this.history.timeBaselines && this.history.timeBaselines.length > 0) {
        ...
        logger.info(
          { timeBaselinesCount: this.history.timeBaselines.length },
          'Saved time baselines to file',
        );
      }
    } catch (error: unknown) {
