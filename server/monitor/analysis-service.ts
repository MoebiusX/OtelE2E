...
import type { Anomaly, AnalysisResponse, JaegerTrace, JaegerSpan } from './types';
...
export class AnalysisService {
    ...
    try {
      ...
      if (this.isOllamaAvailable) {
        ...
        if (!hasModel) {
          logger.warn(
            { model: MODEL },
            'Ollama model not found - pull model to enable AI analysis',
          );
        }
      }
   ...
   */
  async analyzeAnomaly(anomaly: Anomaly, fullTrace?: JaegerTrace): Promise<AnalysisResponse> {
    // Check cache first
...
${spans
  .slice(0, 10)
  .map((s) => `- ${s.service}: ${s.operation} (${s.duration})`)
  .join('\n')}
  ...
  private parseResponse(traceId: string, response: string): AnalysisResponse {
    ...
    const extractBullets = (text: string | undefined): string[] => {
      ...
      return text
        ...
        .filter(
          (line) =>
            line.length > 0 && !line.startsWith('CAUSES') && !line.startsWith('RECOMMENDATIONS'),
        );
    };
