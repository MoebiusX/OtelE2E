...
import type { JaegerTrace, JaegerSpan, SpanBaseline } from './types';
...
export class TraceProfiler {
  ...
  private async fetchTraces(service: string): Promise<JaegerTrace[]> {
    try {
      ...
      const cause =
        error instanceof Error && 'cause' in error ? (error.cause as { code?: string }) : undefined;
  ...
  private updateBaselines(spans: Array<{ span: JaegerSpan; service: string }>): void {
    ...
    for (const [key, durations] of Array.from(spanGroups.entries())) {
      ...
      const variance =
        durations.reduce((sum: number, d: number) => sum + Math.pow(d - mean, 2), 0) / n;
