...
if (!fs.existsSync(INPUT_FILE)) {
  ...
  if (fs.existsSync(altInput)) {
    ...
    console.error(
      '  curl http://localhost:5000/api/monitor/training/export > data/training-data.jsonl',
    );
    process.exit(1);
  }
} else {
...
function convertFromJson(inputPath) {
  ...
  for (const example of data) {
    ...
    const completion = example.rating === 'bad' ? example.correction : example.completion;
