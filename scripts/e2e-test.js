...
async function retryWithBackoff(fn, options = {}) {
  ...
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    ...
    if (attempt < maxRetries) {
      const delayMs = Math.min(initialDelay * Math.pow(backoffMultiplier, attempt - 1), maxDelay);
      await delay(delayMs);
    }
  }
...
function validateSpans(trace, expectedServices, expectedSpanNames) {
  ...
  const services = new Set(
    spans.map((s) => s.processID).map((pid) => trace.data[0]?.processes[pid]?.serviceName),
  );
      ...
      async (attempt) => {
        ...
        if (spans.length >= 3) {
          const services = new Set(
            spans
              .map((s) => s.processID)
              .map((pid) => fullTrace.data[0]?.processes[pid]?.serviceName),
          );

          console.log(); // New line after attempts
          console.log(
            `   ✓ Found correct trace with ${spans.length} spans (took ${Date.now() - paymentTime}ms)`,
          );
...
async function main() {
  ...
  results.push(
    await runTest('Use Case 2: Client Headers (Client provides context)', testCase2_ClientHeaders),
  );
