...
1. **Start the application**:
   ...
   npm run dev
   ```
   *This command checks for Docker, starts external services (Kong, RabbitMQ, Jaeger), and launches the application.*

2. **Open the PoC**: Navigate to `http://localhost:5000`
...
- **RabbitMQ Message Broker**: Message queue with trace correlation across async boundaries  
...
```
Frontend (React) → Kong Gateway → Payment API → RabbitMQ → Consumer
```
...
- **RabbitMQ Spans**: Message broker publish/consume operations with W3C trace context propagation
...
```json
{
  ...
  "spans": [
    ...
    {
      "operationName": "rabbitmq.publish",
      "serviceName": "payment-api",
      "duration": 0.100799,
      "attributes": {
