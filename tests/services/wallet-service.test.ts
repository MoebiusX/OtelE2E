...
describe('Wallet Service', () => {
      ...
      const mockClient = {
        query: vi
          ...
          .mockResolvedValueOnce({
            rows: [{ id: '1', user_id: 'user1', asset: 'BTC', balance: '1' }],
          })
          .mockResolvedValueOnce({ rows: [] }) // transaction insert
          .mockResolvedValueOnce({
            rows: [{ id: '2', user_id: 'user1', asset: 'ETH', balance: '10' }],
          })
          .mockResolvedValueOnce({ rows: [] })
          .mockResolvedValueOnce({
            rows: [{ id: '3', user_id: 'user1', asset: 'USDT', balance: '10000' }],
          })
          .mockResolvedValueOnce({ rows: [] })
          .mockResolvedValueOnce({
            rows: [{ id: '4', user_id: 'user1', asset: 'USD', balance: '5000' }],
          })
          .mockResolvedValueOnce({ rows: [] })
          .mockResolvedValueOnce({
            rows: [{ id: '5', user_id: 'user1', asset: 'EUR', balance: '4500' }],
          })
          .mockResolvedValueOnce({ rows: [] }),
      };
  ...
  describe('getWallet', () => {
    it('should return wallet for valid asset', async () => {
      ...
      expect(db.query).toHaveBeenCalledWith(expect.any(String), ['user1', 'BTC']);
    });
    ...
    it('should normalize asset to uppercase', async () => {
      ...
      expect(db.query).toHaveBeenCalledWith(expect.any(String), ['user1', 'ETH']);
    });
  });
  ...
  describe('credit', () => {
    ...
    it('should throw when wallet not found', async () => {
      ...
      await expect(walletService.credit('user1', 'xyz', 100)).rejects.toThrow(
        'Wallet not found for asset: xyz',
      );
    });
  ...
  describe('debit', () => {
    ...
    it('should throw when insufficient balance', async () => {
      ...
      await expect(walletService.debit('user1', 'btc', 50, 'withdrawal')).rejects.toThrow(
    ...
    it('should throw when wallet not found', async () => {
      ...
      await expect(walletService.debit('user1', 'xyz', 50, 'withdrawal')).rejects.toThrow(
        'Wallet not found for asset: xyz',
      );
    });
  });
  ...
  describe('lockFunds', () => {
    it('should lock funds for pending order', async () => {
      ...
      expect(db.query).toHaveBeenCalledWith(expect.stringContaining('locked = locked + $1'), [
        1000,
        'user1',
        'USDT',
      ]);
  ...
  describe('transfer', () => {
    ...
    it('should throw when sender wallet not found', async () => {
      ...
      await expect(walletService.transfer('sender', 'receiver', 'BTC', 0.5)).rejects.toThrow(
    ...
    it('should throw when insufficient funds', async () => {
      ...
      await expect(walletService.transfer('sender', 'receiver', 'BTC', 1.0)).rejects.toThrow(); // InsufficientFundsError
    });
