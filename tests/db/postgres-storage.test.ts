...
import {
  ...
  closePool,
  testConnection,
} from '../../server/db/postgres-storage';
...
describe('PostgresStorage', () => {
  ...
  describe('Connection Management', () => {
    ...
    it('should handle connection test failure', async () => {
      (mockPool.connect as ReturnType<typeof vi.fn>).mockRejectedValue(
  ...
  describe('User Operations', () => {
    it('should return demo users from getUsers', async () => {
  ...
  describe('Wallet Operations', () => {
    it('should get wallet by demo address', async () => {
      // Mock the database query for demo wallet lookup
      (mockPool.query as ReturnType<typeof vi.fn>).mockResolvedValue({
        rows: [{ user_id: 'test-uuid' }],
      });

      // Get demo wallet address
      const { DEMO_WALLETS } = await import('../../server/storage');
      const aliceAddress = DEMO_WALLETS.alice.address;

      const wallet = await storage.getWalletByAddress(aliceAddress);

      expect(wallet).toBeDefined();
      expect(wallet?.ownerId).toBe('alice@demo.com');
      expect(wallet?.address).toBe(aliceAddress);
    });
    ...
    it('should get wallets by owner', async () => {
      (mockPool.query as ReturnType<typeof vi.fn>).mockResolvedValue({
        rows: [{ user_id: 'test-uuid', email: 'alice@demo.com', created_at: new Date() }],
      });
    ...
    it('should return correct decimals for each asset', async () => {
      (mockPool.query as ReturnType<typeof vi.fn>)
        .mockResolvedValueOnce({
          rows: [{ id: 'wal_btc', balance: '100', updated_at: new Date() }],
        })
        .mockResolvedValueOnce({
          rows: [{ id: 'wal_usd', balance: '100', updated_at: new Date() }],
        })
        .mockResolvedValueOnce({
          rows: [{ id: 'wal_eth', balance: '100', updated_at: new Date() }],
        });
  ...
  describe('Legacy Wallet Operations', () => {
    it('should get legacy wallet by user ID', async () => {
      ...
      const wallet = await storage.getWallet('alice@demo.com');
    ...
    it('should return demo wallet for demo user IDs', async () => {
      (mockPool.query as ReturnType<typeof vi.fn>).mockResolvedValue({ rows: [] });
    ...
    it('should update legacy wallet', async () => {
      ...
      const wallet = await storage.updateWallet('alice@demo.com', { btc: 2.0, usd: 60000 });
  ...
  describe('Transfer Operations', () => {
    it('should create transfer', async () => {
      ...
      expect(transfer.fromAddress).toBe(DEMO_WALLETS.alice.address);
      expect(transfer.toAddress).toBe(DEMO_WALLETS.bob.address);
...
describe('Storage Factory', () => {
  ...
  it('should default to memory storage', async () => {
  ...
  it('should recognize postgresql storage type', async () => {
