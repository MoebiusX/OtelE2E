...
vi.mock('../../server/monitor/anomaly-detector', () => ({
  anomalyDetector: {
    getServiceHealth: vi.fn(() => [{ service: 'kx-wallet', status: 'healthy', anomalyCount: 0 }]),
...
describe('Monitor Routes', () => {
  ...
  describe('POST /api/monitor/analyze', () => {
    it('should return 400 without traceId', async () => {
      const response = await request(app).post('/api/monitor/analyze').send({});
  ...
  describe('POST /api/monitor/correlate', () => {
    it('should return 400 without required fields', async () => {
      const response = await request(app).post('/api/monitor/correlate').send({});
  ...
  describe('POST /api/monitor/training/rate', () => {
    it('should return 400 for missing fields', async () => {
      const response = await request(app).post('/api/monitor/training/rate').send({});
