...
async function main() {
  ...
  try {
    ...
    await channel.consume(ORDERS_QUEUE, async (msg) => {
      ...
      console.log(
        `[MATCHER] Original POST context: ${originalPostTraceparent?.slice(0, 40) || 'none'}...`,
      );
      ...
      const originalContext = {
        traceparent: originalPostTraceparent,
        tracestate: originalPostTracestate,
      };
      ...
      await context.with(trace.setSpan(parentContext, span), async () => {
        try {
          ...
          console.log(
            `[MATCHER] Processing ${side} order: ${quantity.toFixed(8)} BTC @ ~$${currentPrice}`,
          );
          ...
          console.log(
            `[MATCHER] Executed: ${side} ${quantity.toFixed(8)} BTC @ $${fillPrice} (slip: ${slippage}%)`,
          );
          ...
          console.log(
            `[MATCHER] Response with POST context: ${originalContext.traceparent?.slice(0, 40) || 'none'}...`,
          );
